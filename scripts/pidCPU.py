...
try:
    ...
    print("ERROR " + config.logLoco + "/atlassian_PID_cpu.log already exists but this user does not have write access. Adjust permissions or delete existing file")import CONFIG as config
import psutil
import datetime
import time
import socket
...
def takeDumps():
    ...
    for x in range(config.dataCount):
        os.system("top -b -H -p " + str(pid()) + " -n1 > " + config.logLoco + "/atlassian_PID_cpu_usage.`date +%s`.txt")
        os.system("jstack " + str(pid()) + " > " + config.logLoco+ "/atlassian_PID_thread_dump.`date +%s`.txt")
        # Duration between dumps
        time.sleep(config.sleepTime)

def fileList():
    # do an ls on logLoco and grep for conf_ so we know what was collected
    return os.system("ls -al " + config.logLoco + " | grep atlassian_PID_")
...
#
# for x in range (50):
#     print(p.cpu_percent(interval=1.0) // cores)
